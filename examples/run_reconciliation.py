...
def main():
    ...
    ConfigParser.validate_recon_config(config)
    
    print("\n2. Initializing Engine (Pandas)")
    engine = PandasEngine()
    
    print("\n3. Executing Reconciliation")
    reconciler = Reconciler(engine)
    results = reconciler.execute(config)
    
    print("\n4. Generating Report\n")
    report = ReportFormatter.format_reconciliation_report(results)
    print(report)
